...
def main_func():
    ...
    # Here you can change the amount of data you want to start the training.
    t_train = 100
...
def train_func():
    ...
    # I'll use the beginning of the data as state adjuster
    test_data = raw[train_size:, :]

    # the entire train data, except the last shift steps
    x_train = train_data[:-time_shift, :]
    # the entire test data, except the last shift steps
    x_test = test_data[:-time_shift, :]
    # the entire raw data, except the last shift steps
    ...
    # ok shape (1,steps,5) - 1 sequence, many steps, 5 features
    x_train = x_train.reshape(1, x_train.shape[0], x_train.shape[1])
    y_train = y_train.reshape(1, y_train.shape[0], y_train.shape[1])
    x_test = x_test.reshape(1, x_test.shape[0], x_test.shape[1])
    y_test = y_test.reshape(1, y_test.shape[0], y_test.shape[1])
    x_predict = x_predict.reshape(1, x_predict.shape[0], x_predict.shape[1])
    #expected_sequence_length = 319
    # If x_predict has a larger sequence length, trim it
    #if x_predict.shape[1] > expected_sequence_length:
    #    x_predict = x_predict[:, :expected_sequence_length, :]
    #if x_predict.shape[1] < expected_sequence_length:
    #    padding_length = expected_sequence_length - x_predict.shape[1]
    #    x_predict = np.pad(
    #        x_predict, ((0, 0), (0, padding_length), (0, 0)), mode='constant')
    y_predict_true = y_predict_true.reshape(
        1, y_predict_true.shape[0],   y_predict_true.shape[1])
    ...
    model_A = tf.keras.models.load_model('nn.h5')
    ...
    if train_size > 0:
        ...
        plt.plot([train_size, train_size], [minVal, maxVal],
                 label='train/test limit', color='k')
